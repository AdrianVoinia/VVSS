...
public class MemberRepository {
	...
	try{
		...
		while ((currentLineEntry = bufferedReaderEntry.readLine()) != null) {
			...
			int classId=Integer.parseInt(line[1]);
			int valueEntry = Integer.parseInt(line[2]);
			int idEntryMember = Integer.parseInt(line[3]);
			Entry e = new Entry(line[0],classId,valueEntry,idEntryMember);
			this.entries.add(e);			
		}
	 }catch(Exception ex){
         System.err.println(ex.getMessage());
